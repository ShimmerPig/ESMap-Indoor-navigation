window.app = {
	...
	//保存商品类目信息列表
	setProductCategoryList: function(productCategoryList){
		var productCategoryListStr=JSON.stringify(productCategoryList);
		plus.storage.setItem("productCategoryList",productCategoryListStr);
	},
	
	//保存商品信息lsit
	setProductInfoList: function(productInfoList){
		var productInfoListStr=JSON.stringify(productInfoList);
		plus.storage.setItem("productInfoList",productInfoListStr);
	},
	
	//获取本地缓存中的商品类目列表
	getProductCategoryList: function(){
		var productCategoryListStr=plus.storage.getItem("productCategoryList");
		
		if (!this.isNotNull(productCategoryListStr)) {
			return [];
		}
		
		return JSON.parse(productCategoryListStr);
	},
	
	//获取本地缓存中的商品信息list
	getProductInfoList: function(){
		var productInfoListStr=plus.storage.getItem("productInfoList");
		
		if (!this.isNotNull(productInfoListStr)) {
			return [];
		}
		
		return JSON.parse(productInfoListStr);		
	},
